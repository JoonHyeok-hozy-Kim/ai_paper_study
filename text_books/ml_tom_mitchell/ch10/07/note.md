...
*Muggleton, 1992, 1995*
- Objective)
  - Reduce the number of candidate hypotheses generated by the inverse resolution
    - why?)
      - The inverse resolution can generate massive amount of hypotheses.
- How?)
  - Find the most specific hypothesis that can be used to bound a general-to-specific search through the hypothesis space similar to that of [FOIL](../05/note.md#concept-foil).
  - Add the constraint that the only hypotheses considered are hypotheses more general than this bound.
  - The PROGOL system employ this strategy.
...
- Steps)
  ...
  2. Sequential Covering Algorithm
     ...
     - For each example $\langle x_i, f(x_i)\rangle$ not covered by these expressions, PROGOL searches for the most specific hypothesis $h_i \in H$ such that $(B\wedge h_i \wedge x_i)\vdash f(x_i)$.
       - More precisely, it approximates this by calculating the most specific hypothesis among those that entail $f(x_i)$ within $k$ applications of the resolution rule
         - where $k$ is a user-specified parameter
  3. General-To-Specific Search
     ...
     - Within this set of hypotheses, it seeks the hypothesis having minimum description length (measured by the number of literals).
     - This part of the search is guided by an A*-like heuristic that allows pruning without running the risk of pruning away the shortest hypothesis.
